  ...
  return (Array.isArray(candidates) ? candidates : []).map((p) => {
    ...
    return {
      ...
      currency:
        ...
        p?.currency ||
        "KRW",
      sold: toInt(soldRaw), // 지금은 사용 안 하지만 보존
...
async function fetchPopularKR({
  ...
  for (let pageNo = 1; stopNumber === 50; pageNo++) {
    // 1) HMAC + epoch + /rest
    ...
    if (r.items.length) {
      const filtered = r.items.filter((it) => matchesCategory(it, categoryId));
    ...
    console.log(
      "r:",
      r.data?.aliexpress_affiliate_product_query_response?.resp_result?.result
    ...
    stopNumber =
      r?.data?.aliexpress_affiliate_product_query_response?.resp_result?.result
        ?.products?.product.length;
    acc.push(
      ...r.data?.aliexpress_affiliate_product_query_response?.resp_result
  ...
  // return r.data; // 기존 반환 형태 유지
  return [...acc]; // 기존 반환 형태 유지
}

// ===== 실행 예시 =====
(async () => {
  for (let item of categorieList.items) {
    ...
    const list = await fetchPopularKR({
      ...
      pageNo: 1,
      categoryId: 2, // 축산 카테고리 ID 예시 (first_level or second_level 둘 다 매칭 허용)
    });

    // 반환이 배열(정규화된 아이템)일 수도, 원시 응답객체일 수도 있어 안전 처리
    if (Array.isArray(list)) {
      ...
      const count =
        list?.aliexpress_affiliate_product_query_response?.resp_result?.result
          ?.products?.product?.length ?? 0;
      console.log("count(raw):", count);
      console.log(
        list?.aliexpress_affiliate_product_query_response?.resp_result?.result?.products?.product?.slice?.(
          0,
          1
        )
      );
...
//     keywords: "", // 카테고리 전체에서 인기
//     pageNo: 1,
//     categoryId: 201376204, // 축산 카테고리 ID 예시 (first_level or second_level 둘 다 매칭 허용)
//   });
